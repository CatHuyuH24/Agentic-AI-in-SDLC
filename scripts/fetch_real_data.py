...
def download_prices(tickers, start, end):
    ...
    for ticker in ticker_list:
        if len(ticker_list) == 1:
            ...
            ticker_df = raw[ticker].copy() if ticker in raw.columns else raw.xs(ticker, axis=1, level=0).copy()
        ...
        for date, row in ticker_df.iterrows():
            ...
            records.append(
                {
                    "ticker": ticker,
                    "trading_date": pd.Timestamp(date).strftime("%Y-%m-%d"),
